...
export class Jogo {
    ...
    validarSeTerminou() {
        for (let index = 0; index < this.tabuleiro.length; index++) {
            ...
            for (let index2 = 0; index2 < coluna.length; index2++) {
                ...
                let diagonal_inf = this.getCoordenadasDiagonalInferior(index, index2)
                    .map((x) => this.getCelulaInterna(x[0], x[1]))
                    .join("");

                // Ver se alguém venceu na diagonal inferior.
                this.vencedor = diagonal_inf.includes("falsefalsefalsefalse") ? ResultadoJogo.AMARELO :
                    (diagonal_inf.includes("truetruetruetrue") ? ResultadoJogo.VERMELHO : null);
    ...
    */
    tabuleiroCheio() {
        for (let index = 0; index < this.tabuleiro.length; index++) {
            const coluna = this.tabuleiro[index];
            if (coluna.length < Jogo.NUMERO_LINHAS) {
                return false;
            }
        }
        return true;
